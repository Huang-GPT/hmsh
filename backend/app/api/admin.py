...
from app.models.common_fault import (
    FaultCategory, CommonFault, FaultAttachment,
)
...
def get_all_faults():
    ...
    return jsonify({'faults': [f.to_dict(with_attachments=False) for f in faults]})
...
@role_required('admin','operator')
def create_fault():
    """创建故障条目（仅元数据；附件走独立 attachments endpoint）"""
    data = request.get_json()
    fault = CommonFault(
    ...
    db.session.commit()
    return jsonify({'message': '创建成功', 'fault': fault.to_dict(with_attachments=True)})
...
def update_fault(fault_id):
    ...
    db.session.commit()
    return jsonify({'message': '更新成功', 'fault': fault.to_dict(with_attachments=True)})
